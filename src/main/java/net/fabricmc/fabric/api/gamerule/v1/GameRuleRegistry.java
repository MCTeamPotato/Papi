...
public final class GameRuleRegistry {
	 ...
	 */
	@Contract("_, _, _ -> new")
	public static <T extends GameRules.Rule<T>> GameRules.@NotNull Key<T> register(String name, GameRules.Category category, GameRules.Type<T> type) {
	...
	public static boolean hasRegistration(String ruleName) {
		for (GameRules.Key<?> key : GameRulesAccessor.getRuleTypes().keySet()) {
			if (key.getName().equals(ruleName)) return true;
		}
		return false;
	}
}
