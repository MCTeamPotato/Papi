...
public enum BlendMode {
	...
	public static BlendMode fromRenderLayer(RenderLayer renderLayer) {
		return RENDER_LAYER_BLEND_MODE_MAP.getOrDefault(renderLayer, DEFAULT);
	}

	private static final Map<RenderLayer, BlendMode> RENDER_LAYER_BLEND_MODE_MAP = new Object2ObjectOpenHashMap<>();

	static {
		RENDER_LAYER_BLEND_MODE_MAP.put(RenderLayer.getSolid(), SOLID);
		RENDER_LAYER_BLEND_MODE_MAP.put(RenderLayer.getCutoutMipped(), CUTOUT_MIPPED);
		RENDER_LAYER_BLEND_MODE_MAP.put(RenderLayer.getCutout(), CUTOUT);
		RENDER_LAYER_BLEND_MODE_MAP.put(RenderLayer.getTranslucent(), TRANSLUCENT);
	}
}
