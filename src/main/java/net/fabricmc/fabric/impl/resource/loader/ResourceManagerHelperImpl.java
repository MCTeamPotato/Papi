...
public class ResourceManagerHelperImpl implements ResourceManagerHelper {
	 ...
	 * @see ResourceManagerHelper#registerBuiltinResourcePack(Identifier, IModInfo, ResourcePackActivationType)
	 */
	public static boolean registerBuiltinResourcePack(Identifier id, String subPath, IModInfo container, ResourcePackActivationType activationType) {
		Path filePath = ((ModFileInfo)container.getOwningFile()).getFile().getFilePath();
		String separator = filePath.getFileSystem().getSeparator();
		subPath = subPath.replace("/", separator);

		Path resourcePackPath = filePath.resolve(subPath).toAbsolutePath().normalize();
		...
		builtinResourcePacks.add(new Pair<>(name, new ModNioResourcePack((ModInfo) container, resourcePackPath, null, activationType) {
			@Override
