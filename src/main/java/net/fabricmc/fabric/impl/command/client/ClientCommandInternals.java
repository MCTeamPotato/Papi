...
public final class ClientCommandInternals {
	...
	public static boolean executeCommand(String command) {
		...
		try {
			...
			Objects.requireNonNull(activeDispatcher).execute(command, commandSource);
			return true;
	...
	public static void finalizeInit() {
		if (!Objects.requireNonNull(activeDispatcher).getRoot().getChildren().isEmpty()) {
	...
	private static int executeRootHelp(CommandContext<FabricClientCommandSource> context) {
		return executeHelp(Objects.requireNonNull(activeDispatcher).getRoot(), context);
	}

	private static int executeArgumentHelp(CommandContext<FabricClientCommandSource> context) throws CommandSyntaxException {
		ParseResults<FabricClientCommandSource> parseResults = Objects.requireNonNull(activeDispatcher).parse(StringArgumentType.getString(context, "command"), context.getSource());
	...
	private static int executeHelp(CommandNode<FabricClientCommandSource> startNode, CommandContext<FabricClientCommandSource> context) {
		Map<CommandNode<FabricClientCommandSource>, String> commands = Objects.requireNonNull(activeDispatcher).getSmartUsage(startNode, context.getSource());

		for (String command : commands.values()) {
			context.getSource().papi$sendFeedback(Text.literal("/" + command));
		}
	...
	public static void addCommands(CommandDispatcher<FabricClientCommandSource> target, FabricClientCommandSource source) {
		...
		originalToCopy.put(Objects.requireNonNull(activeDispatcher).getRoot(), target.getRoot());
