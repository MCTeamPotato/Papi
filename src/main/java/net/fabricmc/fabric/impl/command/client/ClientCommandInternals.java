...
public final class ClientCommandInternals {
	 ...
	 */
	public static boolean executeCommand(@NotNull String message) {
		if (message.isEmpty()) {
		...
		try {
			...
			Papi.LOGGER.log(ignored ? Level.DEBUG : Level.WARN, "Syntax exception for client-sided command '{}'", message, e);
			...
			Papi.LOGGER.warn("Error while executing client-sided command '{}'", message, e);
	...
	private static Text getErrorMessage(@NotNull CommandSyntaxException e) {
	...
	public static void finalizeInit() {
		...
		DISPATCHER.findAmbiguities((parent, child, sibling, inputs) -> {
			Papi.LOGGER.warn("Ambiguity between arguments {} and {} with inputs: {}", DISPATCHER.getPath(child), DISPATCHER.getPath(sibling), inputs);
		});
	}
	...
	private static int executeHelp(CommandNode<FabricClientCommandSource> startNode, @NotNull CommandContext<FabricClientCommandSource> context) {
	...
	public static void addCommands(@NotNull CommandDispatcher<FabricClientCommandSource> target, FabricClientCommandSource source) {
	...
	private static void copyChildren(
			@NotNull CommandNode<FabricClientCommandSource> origin,
