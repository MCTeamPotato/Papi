...
public abstract class RuleListWidgetVisitorMixin implements GameRules.Visitor, FabricGameRuleVisitor {
	...
	public void visitDouble(GameRules.Key<DoubleRule> key, GameRules.Type<DoubleRule> type) {
		this.createRuleWidget(key, (name, description, ruleName, rule) -> new DoubleRuleWidget(GameRuleReflectionUtils.getGameRulesScreen(this), name, description, ruleName, rule));
	}

	@Override
	public <E extends Enum<E>> void visitEnum(GameRules.Key<EnumRule<E>> key, GameRules.Type<EnumRule<E>> type) {
		this.createRuleWidget(key, (name, description, ruleName, rule) -> new EnumRuleWidget<>(GameRuleReflectionUtils.getGameRulesScreen(this), name, description, ruleName, rule, key.getTranslationKey()));
	}
